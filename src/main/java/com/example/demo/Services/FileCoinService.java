...
import com.example.demo.Repositories.FileCoinRepository;
...
@Service
public class FileCoinService implements ICoinService {

    @Autowired
    private FileCoinRepository fileCoinRepository;

    public FileCoinService() throws IOException {
    }

    @Override
    public List<Coin> getAll() {
        return fileCoinRepository.getAll();
    }

    @Override
    public Coin getAllTimeMax() {
        List<Coin> coins = fileCoinRepository.getAll();
    ...
    public Coin getActual() throws IOException {
        String webPage = "https://min-api.cryptocompare.com/data/v2/histominute?fsym=FIL&tsym=USD&limit=1";
        ...
        if (matcher.find())
            json = matcher.group();
        json = fileCoinRepository.styleJson(json);
        ...
        month = fileCoinRepository.getMonth(month);
    ...
    public Coin getLast() {
        return fileCoinRepository.getAll().get(fileCoinRepository.getAll().size() - 1);
    }
