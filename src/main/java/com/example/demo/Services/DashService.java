...
public class DashService implements ICoinService {
    ...
    public List<Coin> getAll() {
        return dashRepository.getAll();
    }

    @Override
    public Coin getAllTimeMax() {
        List<Coin> coins = dashRepository.getAll();
    ...
    public Coin getActual() throws IOException {
        String webPage = "https://min-api.cryptocompare.com/data/v2/histominute?fsym=DASH&tsym=USD&limit=1";
        ...
        if (matcher.find())
            json = matcher.group();
        json = dashRepository.styleJson(json);
        ...
        month = dashRepository.getMonth(month);
    ...
    public Coin getLast() {
        return dashRepository.getAll().get(dashRepository.getAll().size() - 1);
    }
