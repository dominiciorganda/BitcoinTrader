...
import com.example.demo.Repositories.BitcoinCashRepository;
import com.example.demo.Repositories.LitecoinRepository;
...
@Service
public class BitcoinCashService implements ICoinService {

    @Autowired
    private BitcoinCashRepository bitcoinCashRepository;

    public BitcoinCashService() throws IOException {
    }

    @Override
    public List<Coin> getAll() {
        return bitcoinCashRepository.getAll();
    }

    @Override
    public Coin getAllTimeMax() {
        List<Coin> coins = bitcoinCashRepository.getAll();
    ...
    public Coin getActual() throws IOException {
        String webPage = "https://min-api.cryptocompare.com/data/v2/histominute?fsym=BCH&tsym=USD&limit=1";
        ...
        if (matcher.find())
            json = matcher.group();
        json = bitcoinCashRepository.styleJson(json);
        ...
        month = bitcoinCashRepository.getMonth(month);
    ...
    public Coin getLast() {
        return bitcoinCashRepository.getAll().get(bitcoinCashRepository.getAll().size() - 1);
    }
