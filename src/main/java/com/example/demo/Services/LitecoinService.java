...
@Service
public class LitecoinService implements ICoinService {
    private LitecoinRepository litecoinRepository = new LitecoinRepository();

    public LitecoinService() throws IOException {
    }

    @Override
    public List<Coin> getAll() {
        return litecoinRepository.getAll();
    }

    @Override
    public Coin getAllTimeMax() {
        List<Coin> coins = litecoinRepository.getAll();
    ...
    public Coin getActual() throws IOException {
        String webPage = "https://min-api.cryptocompare.com/data/v2/histominute?fsym=LTC&tsym=USD&limit=1";
        ...
        if (matcher.find())
            json = matcher.group();
        json = litecoinRepository.styleJson(json);
        ...
        month = litecoinRepository.getMonth(month);
    ...
    public Coin getLast() {
        return litecoinRepository.getAll().get(litecoinRepository.getAll().size() - 1);
    }
